...
<body>
    ...
    <div id="voltar">
        <a id="voltar" class="btn btn-default btn-sm float-right" href="gerenciaralunos.php">Voltar</a>
    </div>
    <div class="form-group" style="padding-bottom: 2%;">
        ...
        <form action="#" method="post">
            <select name="select1" id="select-avaliacao">
                ...
                <?php while($row = mysqli_fetch_aarray($result)){ ?>
                    <option value="<?php echo $row['id_avaliacao'] ?>"><?php echo $row['data_avaliacao'] ?></option>
                <?php } ?>
            </select>
            <select name="select2" id="select-avaliacao">
                <?php 
                $sql = "SELECT * FROM avaliacao";
                $result = mysqli_query($conn, $sql);
                ?>
            ...
            <input type="submit" id="" class="btn btn-dark btn-sm" name="buscar" value="Buscar">
        </form>
